...
Object.assign(Whiteboard.prototype, {
  ...
  onAccessKey: function onAccessKey (auth, requests, response, tryNum) {
    ...
    .then(function checkAccessKey (res) {
      if (res.accessKeyId && res.accessKey && tryNum < 1) {
        ...
        if (typeof this.getAccessKey === 'function') {
          var cb
          return new Promise(function (resolve, reject) {
            ...
            var promise = this.getAccessKey(cb)
          ...
          .then(function saveCb () {
            this.accessKeyId = res.accessKeyId
            this.accessKey = res.accessKey
            this.baseUrl = res.baseUrl || this.baseUrl
            this.headersPrefix = res.headersPrefix || this.headersPrefix
  ...
  apiInit: function apiInit () {
    this.api = ddvRestfulApi.getApi(function setConfig (api) {
      api.setBaseUrl('http://yun.linyue.hznwce.com/')
  ...
  testApi: function () {
    ...
    this.api.get('v4_0/whiteboard/test')
    .then(function (res) {
      console.log('111', res)
    })
    .catch(e => {
      // console.log('e', e)
      console.log('e11', e.body)
    })
  }
})
